	...
	if (uMsg == WM_COMMAND)
	{
		switch (LOWORD(wParam))
		{
			case IDOK:
				...
				break;
			case IDC_HOMEPAGE:
				ShellExecute(NULL, NULL, "http://www.camstudio.org/", NULL, NULL, SW_SHOW);
				break;
			case IDC_EMAIL:
	...
	else if (uMsg == WM_COMMAND)
	{
		switch (LOWORD(wParam))
		{
			case IDOK:			
			{
				...
				{
					sprintf_s(algostr,sizeof(algostr),"0");
				}

				else if (IsDlgButtonChecked(hwndDlg, IDC_RADIO2))
				{
					sprintf_s(algostr,sizeof(algostr),"1");
				}
				...
				sprintf_s(gzip_level_out_str,sizeof(gzip_level_out_str),"%d",val);
...
DWORD CodecInst::CompressRGB(ICCOMPRESS* icinfo)
{
	...
	// v1.01 hack to solve bitmaps of width not divisible by 4
	int remainder = icinfo->lpbiInput->biWidth % 4;  
	if (remainder > 0)
	{
		int newbit = icinfo->lpbiInput->biBitCount;	  

		// Do not subtract remainder from here, it actually causes a black line to appear over videos...
		// ~Jake P.
		int newwidth = icinfo->lpbiInput->biWidth; // -remainder; //q: when when the line-len is changed...it works?
