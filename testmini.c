 ...
 */
	...
	bird = malloc(IN_LEN);
	lzo_memset((lzo_byte *) bird,0,in_len);		
	//lzo_memset( bird,0,in_len);		

/*
 * Step 3: compress from `in' to `out' with LZO1X-1
 */
	//r = lzo1x_1_compress(in,in_len,out,&out_len,wrkmem);	
	//r = lzo1x_1_compress((lzo_byte*) bird,in_len,out,&out_len,wrkmem);
	r = lzo1x_1_compress(bird,in_len,out,&out_len,wrkmem);
	free(bird);
